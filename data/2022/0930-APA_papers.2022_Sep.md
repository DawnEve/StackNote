...
# U1 snRNP蛋白通过直接与3'端处理核心因子相互作用，促进选择近端polyA 位点 (J Mol Cell Biol, IF8.1, 2022)
...
真核生物中，splicing和APA都在基因调控网络中起重要作用。
...
本文：敲低或过表达这几个基因(SNRPA, SNRPC, SNRNP70, and SNRPD2)，都促进使用近端polyA位点。
...
【相转移】
SNRNP70 能促进 PABPN1(Poly(A) Binding Protein Nuclear 1) 从液滴变为聚集，从而降低 PABPN1 对近端polyA位点的抑制作用(，从而更多的使用远端)。
...
另外，SNRNP70 也能通过招募 CPSF6 促进近端polyA，表明 CPSF6 对APA的作用和其他RNA结合蛋白相关，并与细胞环境有关。
...
结论：与 U1蛋白复合体不同，自由的U1蛋白可以通过与3’端处理机器互作，促进近端APA位点。
...
> code: https://github.com/imsjay/scAPA_FCA_paper/tree/main/FlyCellAtlasForAlt3p-main
...
3UTR是转录后调控的重点区域，APA可以让这些成为条件调控的，也可以通过最后一个外显子序列改变蛋白序列。
...
这里使用 果蝇细胞图谱 计划的单核RNA测序数据，分析细胞类型(>250个)特异的3UTR。
...
揭示了模式： 分化的神经中3UTR 变长，精子形成中 3UTR缩短。
...
最后，在几个细胞类型中，把 RBP，miRNA 和 CPA因子等做相关分析，发现特有的APA谱图，可能是转录组复杂性的候选调控子。
...
> 55.Goering, R. et al. LABRAT reveals association of alternative polyadenylation with transcript localization, RNA binding protein expression, transcription speed, and cancer survival. BMC Genomics 22, 476 (2021).
...
> 加抑制剂后，交联，磁珠捕获polyA mRNA，酶处理(?)，SDS-PAGE后打质谱。


CPEB4 和 特定mRNA的3’UTR的胞内polyA元件 相关，
并组装成活性复合体来 通过APA促进靶mRNA的翻译。
...
Results: Here, we find that CPEB4 is part of an alternative repressor complex that mediates mRNA degradation by associating with the evolutionarily conserved CCR4-NOT deadenylase complex. 
CPEB4是一个RBP，抑制HDAC(组蛋白去乙酰化酶)时，它与polyA的联系更强。 而抑制 HDAC 是造成带polyA的mRNA广泛降解的已知原因。
We identify human CPEB4 as an RNA-binding protein (RBP) with enhanced association to poly(A) RNA upon inhibition of class I histone deacetylases (HDACs), a condition known to cause widespread degradation of poly(A)-containing mRNA. 

使用内生带标签的CPEB4的HeLa细胞，光激活 核糖核苷增强交联 + 免疫共沉淀 分析表明: CPEB4倾向结合到早期基因的3UTR上，在靠近polyA位置、通常U-或A-丰富的CPE位置、包含G-的变体位置。
Photoactivatable ribonucleoside-enhanced crosslinking and immunoprecipitation (PAR-CLIP) analysis using endogenously tagged CPEB4 in HeLa cells reveals that CPEB4 preferentially binds to the 3'UTR of immediate early gene mRNAs, at G-containing variants of the canonical U- and A-rich CPE located in close proximity to poly(A) sites. 

转录组mRNA降解测试，CPEB4结合与短mRNA半衰期强相关，CPEB4不表达导致早期基因的稳定。
By transcriptome-wide mRNA decay measurements, we find that the strength of CPEB4 binding correlates with short mRNA half-lives and that loss of CPEB4 expression leads to the stabilization of immediate early gene mRNAs. 

类似 CPEB4，我们展示了 CPEB1和CPEB2也通过招募CCR4-NOT 复合体导致mRNA不稳定。
Akin to CPEB4, we demonstrate that CPEB1 and CPEB2 also confer mRNA instability by recruitment of the CCR4-NOT complex.

结论: CPEB4 已知能促进胞内polyA，我们的发现建立了该基因的另外功能：作为一个抑制复合物的RNA接头，增强短半衰期的早期基因mRNA的降解。
...
> Conclusions 第一句话什么意思？胞内腺苷酸化? //todo //疑惑
> CPEB4 was previously known for its ability to stimulate cytoplasmic polyadenylation
> PEB4先前因其刺激细胞质多聚腺苷酸化的能力而闻名
...
本文：m6 A reader 基因 YTHDC1，可以抑制 近端APA位点，通过结合到上游m6A位点来产生长3-UTR转录本。
...
YTHDC1 直接互作 3'端处理因子 FIP1L1，干扰其招募 CPSF4 的能力。
